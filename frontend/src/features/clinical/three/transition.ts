...
/* Какая сцена играется при смене состояния зуба (B7, ступень 6) — чистый
   ...
   удалённый зуб выходит из лунки и растворяется; отсутствующий тает до
   призрака; коронка (и тело моста) садится или «вдыхает»; живой зуб
...
/** Стартовая прозрачность коронки при возврате: из призрака — его 0,22, иначе с нуля. */
export const startOpacity = (from: From): number => (from === 'ghost' ? 0.22 : 0)

/** Откуда поднимается коронка при возврате, мм по оси зуба: из призрака — с места. */
