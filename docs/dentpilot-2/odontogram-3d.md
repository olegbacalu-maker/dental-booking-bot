...
### Под палец: планшет у кресла (26.09.2026)

Слово Олега (через сессию «сетевая настройка»): планшет врача над креслом,
iPad и Android — «надо сделать на оба». Режим «у кресла» не утверждён; здесь
только то, чтобы одонтограмма (2D и 3D) работала пальцем с самого начала.
Всё включается `(pointer: coarse)`, мышью рабочий стол прежний.

- `clinical/touch.ts`: `useCoarse`, `useLongPress` (палец/перо, 500 мс, сдвиг
  10 px отменяет), `swallowNextClick`, `slopOf` (10 px палец / 4 px мышь).
  ⛔ Меню зуба пальцем — ДОЛГОЕ НАЖАТИЕ на pointer-событиях: Safari на iPad
  contextmenu на нём не шлёт. Android шлёт — после сработавшего нажатия он
  гасится. Первый click после нажатия гасится на документе: у края окна меню
  сдвинуто, и отпускание пальца попало бы в пункт меню.
- `components/menu.ts`: меню закрывает `pointerdown` мимо, не `mousedown` —
  замер в эмуляции iPad: меню открылось на +511 мс и закрылось совместимым
  mousedown отпускания.
- Инспектор после касания зуба — ПОВЕРХ дуги и всегда на экране: шторка снизу
  в портрете (до 56 % высоты, зуб прокручивается над ней), панель сбоку в
  альбоме — со стороны, где выбранного зуба нет (сжатая панелью дуга во
  вкладке фиши давала зубы ~25 px). Кнопка «Închide» снимает выбор и черновик.
  Без выбора инспектор пальцем скрыт. `data-sel`/`data-side` на `.odop`.
- Состояние зуба пальцем — крупные кнопки с цветом палитры вместо списка.
- Во вкладке фиши открытие Odontogramă/Parodontogramă ставит полосу вкладок под
  верхнюю панель: на альбомном iPad шапка пациента занимала весь экран.
- 3D: порог касания 10 px, долгое нажатие по коронке → то же меню.
- Цели ≥ 44 px: переключатель видов, «Punte nouă», переключатели 3D, строки меню.
- Стенд `scripts/tablet_shots.py` (сессия «сетевая настройка», порт 9483): до —
  инспектор за краем во всех случаях вкладки и в альбоме страницы, долгое
  нажатие меню не открывало; после — 8 из 8 (iPad и Android, обе ориентации,
  вкладка и страница): инспектор на экране, меню открыто. 3D проверено
  эмуляцией с программным WebGL. Зубы ~38 px шириной (ширина рисунка сервера,
  кнопка высокая) — стенд считает их мелкими, как и на исходном замере.
  ⚠️ Это Chromium с эмуляцией касаний, не WebKit: первый живой iPad проверит
  остальное (выноска, клавиатура, вьюпорт).
