...
📝 **Предложение от 26.09.2026, ждёт решений Олега** (раздел 1). Ничего из
этого в `src/` и `bot/` ещё не сделано. Что уже есть:
...
1. **Где показываем 3D.** Предложение: третий вид «3D» на детальной странице
   `/admin/patient/:pid/odontograma`, рядом с «Vedere frontală / ocluzală».
   Компактная карточка фиши (`OdontogramCard`) остаётся 2D: там первый кадр
   без ожидания и мало места. Печать 043/e, лист пародонтограммы и выгрузка
   не меняются.
2. **Поправка к контракту.** В clinical-chart.md стоит ⛔ «второго движка
   ...
   печать, 043/e и выгрузку 3D не попадает никогда. Записать одним абзацем в
   clinical-chart.md, когда решение принято.
3. **Как везём three.js** (≈ 650 КБ min, ≈ 160 КБ gzip).
   *Вариант B, рекомендуемый:* отдельный файл `bot/app/static/js/three.js`,
...
4. **Дуга 2D подковой и схема MODVL** из макета — отдельные этапы, с 3D не
   ...
   сервер (`teeth_svg.schema_svg`), иначе она не попадёт в печать.
...
- `OdontogramScreen.tsx`: при `c.view === '3d'` в `.odop-main` вместо
  `<DentalArch>` рендерится `<Odontogram3D>` с теми же `onSelect`,
  `onSurface`, `onMenu`; инспектор, контекстное меню, режим моста и
  клавиатура не меняются (стрелки → `c.select`, подсветка в 3D через
  `selected`). `focusTooth` для 3D фокусирует canvas.
- `OdontogramCard.tsx`: карточка знает два вида — сохранённый `'3d'`
  показывать как `'frontal'`, а не падать.
...
| `frontend/vite.config.ts`, `.gitignore`, `Build-Desktop.ps1` | копия three.js, игнор, проверка сборки |
...
| `frontend/src/features/clinical/ViewSwitch.tsx`, `OdontogramScreen.tsx`, `OdontogramCard.tsx` | третий вид |
