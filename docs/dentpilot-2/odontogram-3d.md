...
### Ступень 1 — сервер: размеры и палитра в модели ✅ 26.09.2026
...
- Сделано 26.09: `tooth_geom` отдаёт `md`, `bl`, `crown`, `root`, `roots`, `cls`,
  `upper` (высота коронки и длина корней тоже с сервера; у молочных ×0.8);
  типы `ToothGeom`/`palette` в `chart.ts` пока необязательные — станут
  обязательными, когда их прочитает 3D (ступень 3–4). Пины: `test_teeth.suite_geom`
  (16 → 10,5 мм и три корня, 14 → два, 41 уже 11, 55 → три и коронка ниже, все 52
...
- ⚠️ Поправка вечером 26.09: `three.module.min.js` r180 НЕ один файл — он
  импортирует `./three.core.min.js` (ядро вынесено с r170). Имя с точками маршрут
  `/static/{kind}/{name}` не пропускает, поэтому плагин копирует ядро как
  `three-core.js` и переписывает указатель внутри `three.js`; sourceMappingURL
  снимаются. Первый живой запуск в песочнице показал это как «3D nu s-a
  încărcat» с 404 на `three.core.min.js` — сборка и дымовой тест проверяют
  теперь оба файла.
...
### Ступень 4 — сцена и компонент ✅ 26.09.2026
...
### Ступень 5 — интеграция в экран ✅ 26.09.2026
...
Сделано 26.09 (ступени 4–5): `three/scene.ts` — `createArchScene({THREE, …})` на
загруженном THREE: обе дуги по `layoutArch`, десна, пять материалов на
коронку, корни, винт (+ полупрозрачная копия поверх десны), кольца отметки /
импланта / выбора, номера спрайтами; кадры по требованию (`invalidate`),
своя орбита (перетаскивание, колёсико, щипок, пресеты `VIEWS`), пикинг по
`face.materialIndex` → буква, правая кнопка → меню; `setModel` красит через
`targetLook` (смена структуры пока мгновенно — сцены в ступени 6).
`three/Odontogram3D.tsx` — узел, жизненный цикл, панель видов и
переключателей, статусы loading/failed/nogl, надпись про молочные. Ступень 5:
`View` с `'3d'`, `savedView`, третья кнопка `ViewSwitch`, в
`OdontogramWorkbench` при `c.view === '3d'` вместо `DentalArch` — `Odontogram3D`
с теми же `onSurface`/меню (`onMenuAt`); 2D-компоненты для `'3d'` рисуют
frontal. Тесты: `OdontogramScreen.test.tsx` › «вид 3D» (кнопка, память вида,
отказ загрузки с живым инспектором, сохранённый вид, надпись про молочные).
Кадры headless Edge — после ступени 6, вместе с анимациями.

### Ступень 6 — анимации ✅ 26.09.2026
...
Сделано 26.09: `three/transition.ts` — чистый выбор сцены (`sceneFor`:
имплант / удаление / призрак / золото / живой; `comesFrom`: из лунки,
из-под импланта, из призрака; длительности `MS` и старты возврата) с
Vitest; в `scene.ts` — `animateStruct` (винт вкручивается сверху 900 мс и
коронка садится на него; удалённый выходит из лунки и растворяется, лунка
проявляется; отсутствующий тает до 0,22; золото садится или «вдыхает»; живой
возвращается — винт выкручивается, лунка гаснет, корни проявляются), в конце
каждой сцены `applyLook(…, true)` — вид сходится с данными, даже если твин
сняли следующей сменой (`tweens.kill(t)`). Кадры — через `createTweens` с
