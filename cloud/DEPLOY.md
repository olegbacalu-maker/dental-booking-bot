...
(первое время так и держать); `auto` — файл уходит сразу. Тот же режим — у
заявок из программы (`/v1/trial`): в `approve` программа клиники ждёт на
странице активации и включается сама через минуту после «Выдать». Пробный — месяц
