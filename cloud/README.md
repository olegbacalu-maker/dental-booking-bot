...
кнопкой «Выдать»), дальше — обычное автообновление. Повтор — 409 со словами
«уже зарегистрирована, активируйте файлом из письма» (программе нечем
активироваться, а законный повтор — новый ПК); «Скрыть» отзывает токен, и
программа получает 401. Без `DP_BASE_URL` на https вход отвечает 503: файл
без `renew` программа сама не заберёт.
