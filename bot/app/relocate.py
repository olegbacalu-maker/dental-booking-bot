...
# Раскладка ЛЮБОГО корня — здесь, и только здесь. Внутри работающей программы
# то же место считает `paths.db_dir()` от `$DATABASE_URL`; для ЧУЖОГО корня
# такого ответа нет и быть не может — его никто не запускал. Значит знание
# «в корне есть data\» обязано жить в одном месте, иначе второй вычислитель
# однажды заглянет мимо и доложит «картотеки нет» про живую картотеку.
DATA_REL = "data"
DB_REL = os.path.join(DATA_REL, "dental.db")
