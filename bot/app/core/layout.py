...
MSG_BANNER = {
    ...
    # не работает»). Совет «номер второго — в notițe» опирается на то, что поиск
    # находит цифры и в заметках (_pl_match_q, 08-20).
    "ok_tel_dup": ("warn", "Fișa a fost salvată, dar acest număr de telefon "
                           "există deja la alt pacient. Dacă sunt persoane "
                           "diferite cu un telefon comun, treceți numărul în "
                           "notițele fișei, nu în câmpul Telefon"),
...
_I = {  # компактные stroke-иконки сайдбара
    ...
    # пациент «fără telefon» (08-20): метка обязана быть ВИДНА в списке и фише,
    # а не выводиться из пустой ячейки — иначе «почему этим не уходят SMS»
    # станет загадкой, когда пакет заработает
...
# ⚠️ Узкий сайдбар — ПАРАМЕТР, а не набор страниц рядом с LIVE_RELOAD. Тот
# ключуется по `active`, а `active` отвечает ещё и за подсветку пункта меню:
# детальная одонтограмма обязана держать «Pacienți» подсвеченным и при этом
# идти с узким сайдбаром, и своим ключом она бы подсветку потеряла.
...
# Кто живёт сам — СЕКУНДЫ опроса в data-reload на <body>, panel.js слушается
# атрибута. Живым обязано быть только расписание (панель, день, неделя, день
# врача): бронь из бота должна появиться, пока журнал висит открытым на стойке.
# Остальным страницам это не даёт НИЧЕГО (находка Олега 08-07: «вкладка
# закрывается сама» — третий случай одной болезни после предпросмотра 1.13.2 и
# анимаций). Новая страница расписания = её active сюда.
# ⭐ С 08-20 «живой» значит ОПРОС, а не перезагрузка (жалоба пилота «обновление
# каждые 12 сек, видно»): _shell оборачивает body живой страницы в
# <div id="live" data-hash=md5>, panel.js раз в 12 с спрашивает ТОТ ЖЕ адрес с
# заголовком X-DP-Live и подменяет содержимое обёртки ТОЛЬКО если отпечаток
# изменился (schedule/routes._live_fragment отвечает 204 на совпавший). Экран
# неподвижен, пока день не изменился по-настоящему. Отсюда два следствия:
# страница живой раскладки обязана рендерить body ДЕТЕРМИНИРОВАННО (всё
# непрерывно меняющееся — как линия «сейчас» — рисует panel.js, иначе отпечаток
# «всегда другой» и подмена идёт каждый опрос), а инлайн-скрипт, несущий ДАННЫЕ
# (CARDS, NOTE_ENDS), помечается data-live и объявляет var, не const — panel.js
...
# ---- React-экраны (DentPilot 2.0, 09-17; интерфейс продукта с 21.09) ----
# ⭐ **REACT_SCREENS — это список РЕАЛИЗОВАННЫХ поверхностей, а не механизм
# выката.** Решение Олега 21.09: программы на целевой машине ещё нет, живых
# профилей нет, сохранять нечего — значит и выката нет. Поверхность, здесь
# названная, отдаётся React ПО УМОЛЧАНИЮ.
# ⛔ Не заводить обратно ни канареечных списков, ни поэтапного включения, ни
# rollout-флагов: сложность, которая обслуживает несуществующих пользователей.
#
# Ключ `ui.react` в профиле остаётся, но смысл у него теперь ОБРАТНЫЙ — это
# аварийный выключатель и шов для стендов, а не рубильник выката:
#     ключа нет          → React (так у всякой новой установки)
#     "react": []        → старый интерфейс везде
#     "react": ["имя"]   → только названные (этим пользуются наборы)
# Оба интерфейса лежат в одном exe, поэтому ни включение, ни откат не требуют
# сборки: правка файла (профиль читается при старте и при сохранении настроек)
# либо `?ui=legacy` в адресе, который возвращает старую страницу на один
# запрос — это выход для регистратуры, если новый экран подвёл.
...
REACT_SCREENS = frozenset({"settings_clinic", "doctors_list", "doctor_card",
                           ...
                           "settings_crypt", "settings_system", "stats",
                           "patients_search", "patient_card", "visit", "odontogram",
                           "perio", "schedule_week", "schedule_all",
                           "schedule_doctor",
                           # ⚠️ Панель дня: живой опрос и React несовместимы по
                           # устройству, поэтому страница с узлом живой не
...
def react_flag(screen: str) -> bool:
    ...
    if screen not in REACT_SCREENS:
        return False
    ui = eng.CONFIG.get("ui") or {}
    if "react" not in ui:
        return True                       # новая установка: React и есть продукт
    flags = ui["react"]
    if not isinstance(flags, (list, tuple, set)):
        # ⚠️ `"react": false` — понятная человеку запись, и падать на ней
        # `screen in False` нельзя: у клиники это был бы отказ всей админки.
        return bool(flags)
...
# центрирования: на 27" замер давал 844px пустоты одним куском справа у
# пациентов, врачей, статистики и настроек. Список снят, а не дополнен: ширина
# теперь у самой `.content` (panel.css), потому что ключ ставится в модуле, и
# следующая страница молча родилась бы узкой — а увидел бы это только клиент с
# большим монитором, на 1366 потолок не срабатывает вовсе.
# ⚠️ Ограничение, из-за которого это не однострочник, никуда не делось: ширина
# окна хороша ТАБЛИЦАМ и вредна СПЛОШНОМУ ТЕКСТУ. Потолок переехал к самому
# тексту — переменная `--measure` в panel.css (.hint, .banner, карточка FAQ).
