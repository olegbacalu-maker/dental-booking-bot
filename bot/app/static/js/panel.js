...
/* ⛔ ПЕРВОЙ строкой файла, и это не вкус.
   Страница, отданная React-ом, рисует расписание САМА, а куски ниже ищут её
   узлы по классам и атрибутам — и находят React-овские. Поэтому у каждого
   ЭКРАННОГО куска стоит охрана `if (DP_REACT) return`, и она обязана читать
   уже ПОСЧИТАННОЕ значение: `var` поднимается, значение — нет, и охрана в
   куске выше объявления прочитала бы `undefined`. Выглядела бы написанной и
   не работала.
   ⚠️ Скрипт подключён В КОНЦЕ body (`core/layout`), поэтому `#root` к этому
   моменту уже в документе.
   ⚠️ Каркасные куски (Ctrl+K, часы сайдбара, плашка, `dp_pos`) охраны не
   несут намеренно: они ищут узлы по `id`, которые печатает сам каркас, и
...
(function () {
  /* ⛔ На React-поверхности перетаскивание принадлежит React целиком (C26.5.3-f).
     До этого шага кусок молчал ПО СЛУЧАЙНОСТИ: у React-панели нет узла
     `movedlg`, и выход стоял строкой ниже. Случайность снята здесь, а с другой
     стороны — решением не давать React-диалогу переноса никакого `id`: два
     обработчика на одном броске открыли бы два диалога и отправили бы две
     команды, а увидеть это можно только руками. */
  if (DP_REACT) return;
...
/* ⛔ Три функции ниже ищут узлы сетки по классам и атрибутам — и находят
   React-овские. Кусается ровно одна: `placeNowline` висит на 30-секундном
   интервале и начала бы доклеивать ВТОРУЮ линию внутрь поддерева, которым
   владеет React. Остальные две безвредны ПО СЛУЧАЙНОСТИ: `markFresh` зовётся
   только при загрузке и из подмены живого куска, которой у React-страницы
   нет, а `paintWaits` ищет `data-wait-since`, который React не печатает.
   Случайность — не основание; охрана стоит у всех трёх (и у переноса выше). */
function markFresh() {
  if (DP_REACT) return;
