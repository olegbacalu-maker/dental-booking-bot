...
def render() -> str:
    ...
    items = [
        ...
        _q(_ic("box"), "Cum deschid arhiva de rezervă fără DentPilot?",
           ...
           f"{p}{_ic('ban')} Dacă pe calculator este activată <b>criptarea "
           f"evidenței</b> (există fișierul <b>data\\db.key</b>), la "
           f"restaurarea <b>din arhivă</b> ștergeți și <b>data\\db.key</b>: "
           f"baza din arhivă nu este criptată, iar cu cheia rămasă pe loc "
           f"programul ar deschide-o ca pe una criptată și ar da eroare la "
           f"pornire. După restaurare activați criptarea din nou din "
           f"<b>Setări › Criptarea evidenței</b>. Este singura situație în "
           f"care acest fișier se șterge — la copiile zilnice din "
           f"<b>data\\backups</b> cheia rămâne pe loc: ele sunt criptate cu "
           f"aceeași cheie.{end}"
        ...
        _q(_ic("phone"), "Pot deschide registrul de pe telefon?",
           ...
           f"alegeți «Adaugă pe ecranul principal».{end}"
           f"{p}Pe <b>iPhone</b> registrul se deschide după aceea ca o "
           f"aplicație adevărată — pe tot ecranul, cu iconița programului. Pe "
           f"<b>Android</b> rămâne deocamdată o scurtătură care se deschide în "
           f"browser: pentru mai mult ar fi nevoie de o legătură criptată, iar "
           f"în rețeaua locală ea nu există. Diferența e de sistem, nu de "
           f"telefon — pe amândouă se lucrează la fel.{end}"
        ...
        _q(_ic("folder"), "Unde sunt datele clinicii și ce nu trebuie atins?",
           ...
           f"calculator nu poate fi citit — de aceea foaia. Singura excepție: "
           f"restaurarea unei <b>arhive de rezervă</b> — vezi întrebarea "
           f"despre arhivă.{end}"
