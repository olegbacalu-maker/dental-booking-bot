...
function fitGrid() {{
  ...
  /* ВАЖНО: отступ сверху считается ОТ ДОКУМЕНТА (плюс scrollY), а не от окна.
     getBoundingClientRect().top живёт в координатах ОКНА и меняется от одной
     лишь прокрутки: у прокрученной вниз страницы он уходит в минус, разность
     растёт, и час раздувается. Событие resize при этом приходит само —
     например когда появляется полоса прокрутки, — так что «колонки вырастают
     сами собой при прокрутке» получалось на ровном месте (замечено 08-17). */
  /* ВАЖНО: обе опоры снимаются СЕЙЧАС, пока час стоит на полу, и обе — в
     координатах ДОКУМЕНТА (плюс scrollY). Тогда расчёт не зависит ни от
     прокрутки, ни от собственного результата: fitGrid, запущенный дважды подряд,
     обязан дать то же число. Меряя зазор ПОСЛЕ того, как сам растянул сетку,
     получаешь обратную связь — час подрастал на пиксель за каждый вызов. */
  var rail = document.querySelector('.rail');
  var sy = window.scrollY;
  var top0 = gb.getBoundingClientRect().top + sy;
  /* ВАЖНО: конец СОДЕРЖИМОГО рельса, а не его коробки. Рельс — элемент того же
     flex-ряда и растягивается вслед за сеткой: растянули сетку — коробка рельса
     поехала за ней, и следующий вызов насчитал бы час на пиксель больше, потом
     ещё. Последняя карточка своей высоты не меняет и служит неподвижной опорой. */
  var last = rail && rail.lastElementChild;
  var railEnd = last ? last.getBoundingClientRect().bottom + sy
              : (rail ? rail.getBoundingClientRect().bottom + sy : 0);
  /* Сетка ДОТЯГИВАЕТСЯ до низа правой колонки (08-17, замечание Олега): рельс
     длиннее сетки на 220-240px, и под сеткой висело белое поле, читавшееся как
     незаконченный экран. Берём большее из «до низа окна» и «до низа рельса».
     ВАЖНО: с потолком 96px. У клиники с коротким днём (три-четыре часа) рельс
     той же длины раздул бы час до сотен пикселей, и один визит занял бы
     пол-экрана. */
  var avail = Math.max(window.innerHeight - top0 - 24, railEnd - top0);
  var c = Math.max(66, Math.min(96, Math.floor(avail / n)));
  ...
  fitAppts();
  /* Кто менял высоту часа, тот перекладывает линию «сейчас» (08-21, нашёл
     Олег): первый прогон идёт при парсинге, когда правой колонки ещё нет в
     DOM, час встаёт на пол 66, и линия panel.js ложится по нему; прогон на
     DOMContentLoaded растит час до 92-96, ряды уезжают вниз, а линия
     оставалась в старых пикселях на полтора часа выше — до 30-секундного
     интервала. Охрана обязательна: при парсинге panel.js ещё не исполнен. */
  if (window.placeNowline) placeNowline();
}}
