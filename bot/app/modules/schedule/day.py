...
def birth_max() -> str:
    ...
    ⛔ Не `date.today()`. Поле уехало в живой конверт (`slotform`), а значит в
    отпечаток: считанное по часам машины, оно переворачивало бы отпечаток в
    полночь по чужому поясу и расходилось бы в прогоне под `TZ=UTC0` с
    остальной моделью, которая вся живёт в `eng.TZ`.
    ⚠️ Одна функция на форму страницы, модель дня и конверт панели: три
    «сегодня» в трёх местах разошлись бы ровно в полночь и ровно на сутки.
    """
    return datetime.now(eng.TZ).date().isoformat()


def slot_form(d: date) -> dict:
    """Что нужно диалогу пустого часа на ПАНЕЛИ — и только это.

    ⛔ Имя `slotform`, а не `form`: у модели дня `form` несёт ещё врачей, их
    часы и предвыбор, и ключ с тем же именем, но меньшим составом — ловушка
    для следующего, кто скопирует форму записи. Панели врачи и часы не нужны:
    врач приходит из колонки, час — из ячейки, получас — переключателем.
    """
    del d  # день приходит параметром ради симметрии с `form_spec`
    return {"services": [{"id": k, "label": v["ro"]} for k, v in eng.SERVICES.items()],
            "birth_max": birth_max()}
...
def appt_view(r, dk: str, cards: dict | None, colors) -> dict:
    ...
    return {
        ...
        # ⛔ Три поля ниже — ровно то, чего диалогу карточки не хватало в
        # блоке, и НИ ОДНИМ больше (C26.5.3-a). Вторая структура «карточка»
        # на живой поверхности не заводится: она несёт ОТМЕНЁННЫЕ записи,
        # которых на канве негде нажать, а всё, что попало в конверт, попало
        # в отпечаток — правка у невидимой записи гнала бы перерисовку, в
        # которой на экране не меняется ничего (разбор — interaction-contract).
        # ⚠️ `doctor` — СНИМОК имени из самой записи, а не имя колонки: визит
        # показывается под тем врачом, под которым записан, и после
        # переименования шапка колонки и диалог говорят РАЗНОЕ. Так и надо.
        "doctor": r["doctor"] or "",
