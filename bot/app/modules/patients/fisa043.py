...
_STATE_ABBR = {"ok": "", "carie": "C", "obturatie": "O",
               "coroana": "Cor", "implant": "Imp", "extras": "E", "lipsa": "A"}
# Отметки печатаются ТЕМИ ЖЕ буквами, но отдельным словарём: «T» с 08-17 не
# состояние, а отметка поверх находки, и в клетке она ДОПИСЫВАЕТСЯ — «C MO T»,
...
# поверхностью, и на бумаге тоже. Одной грамматики клетки («мостовой код —
# первый токен») мало: клетка из ОДНОГО «D» достижима и без моста (здоровый
# зуб с отмеченной поверхностью — surface_map при state вне SURFACE_STATES
# пуст, и буква уходит в хвост без кода; ревью 08-22). Различает СПИСОК:
# легенда привязывает мостовое чтение к зубам, перечисленным строкой
# «Punte dentară» под решёткой, с материалом — ровно то, что врач пишет
# под своей acoladă. Скобку сама решётка не рисует.
...
_LEGEND = {
    "ro": ("C — carie · O — obturație · T — în tratament · Cor — coroană · "
           ...
           "Punte: Co — dinte stâlp, D — corp de punte (primul cod din "
           "celulă, doar la dinții enumerați la «Punte dentară»)"),
    "ru": ("C — кариес · O — пломба · T — в лечении · Cor — коронка · "
           ...
           "только у зубов из строки «Punte dentară»)"),
}
    ...
    def cells(nums, kind, pad=0):
        ...
        for i, n in enumerate(nums):
            ...
            if kind == "nr":
                ...
                t = teeth.get(n) or {}
                code = _STATE_ABBR.get(t.get("state", "ok"), "")
                sf = (t.get("surfaces") or "").strip()
                mk = " ".join(_MARK_ABBR[m] for m in
                              tsvg.mark_list(t.get("marks") or ""))
                ...
                if len(parts) > 1:
                    txt = ", ".join(f"{_STATE_ABBR.get(st, '')} {ls}".strip()
                                    for st, ls in parts)
                    if mk:               # код находки уже первый — отметка в хвост
                        txt = f"{txt} {mk}"
                else:
                    # ⛔ ПЕРВЫЙ токен клетки обязан быть КОДОМ из легенды, а не
                    # буквой поверхности: у этого бланка «O» значит И obturație,
                    # И ocluzal, и клетка «O T» на зубе без единой пломбы читается
                    # по его же грамматике как пломбированный. Массово такие зубы
                    # создаёт САМА миграция 5 (у «în tratament» буквы оставались, а
                    # кода находки после перевода в отметку нет) — то есть врут
                    # подписанные бланки, к которым никто не прикасался.
                    # ⭐ Отсюда правило: нет кода находки — в его место встаёт
                    # отметка, и клетка мигрированного зуба выглядит ТОЧНО так же,
                    # как до обновления («T O»). Миграция не имеет права менять то,
                    # что говорит подписываемый документ.
                    head = code or mk
