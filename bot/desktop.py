...
def main() -> None:
    ...
    # ⭐ Прошлая версия удаляется ЗДЕСЬ, а не в bat обновления (08-10, находка
    # аудита). Раньше bat стирал `.old.exe` сразу, как только новый файл
    # ПОЯВЛЯЛСЯ на диске, — а «файл существует» не равно «файл запускается».
    # Битая докачка, антивирус, съевший exe после подмены, несовместимый
    # бинарник — и на диске не оставалось ни одной рабочей копии программы:
    # клиника получала мёртвые ярлыки в утро рабочего дня.
    # Сюда управление доходит только когда сервер поднялся, база открылась и
    # /health ответил НАШЕЙ программой — это самое сильное доказательство «эта
    # сборка работает», какое у нас есть. Не доехали — `.old.exe` остался
    # лежать рядом, и его достаточно переименовать обратно.
    if getattr(sys, "frozen", False):
        try:
            for old in pathlib.Path(sys.executable).resolve().parent.glob("*.old.exe"):
                old.unlink(missing_ok=True)
                logging.warning("Прошлая версия убрана: %s", old.name)
        except OSError as e:
            # не смогли — не беда: лишний файл рядом безвреден, а падать
            # из-за уборки на старте программы недопустимо
            logging.warning("Прошлая версия не убрана (%r)", e)
