...
def suite_backup(res: Result) -> None:
    ...
    # ⚠️ Раскладку задаёт ЛАУНЧЕР через $DENTART_DATA_DIR, и без неё
    # `paths.data_root()` пусто — то есть ветка, которую получает клиника,
    # в прогоне не исполняется вовсе, а проверка «инструкция называет папку»
    # проверяла бы молчание. Ставим переменную здесь; больше её не читает
    # никто, кроме текстов (`layout.data_folder`, `backup._readme`).
    # ⛔ Папку клиники уводим в сторону НАРОЧНО, и двигать надо именно её.
    # По умолчанию на стенде совпадают ВСЕ ТРИ пути — `paths.data_root()`,
    # `paths.db_dir()` и `eng.config_path().parent`, — поэтому проверка
    # «текст называет настоящую папку» зеленеет при любом источнике, в том
    # числе неверном. `paths.db_dir` про это и предупреждает: у клиники
    # `C:\ProgramData\DentPilot` и `…\data` — разные места, а совпадение
    # на стенде есть следствие раскладки, а не определение.
    # ⚠️ Развести одну только БАЗУ мало: `CLINIC_CONFIG` остаётся в `srv.dir`,
    # значит `config_path().parent` так и равен `data_root()` — а подмена
    # источника именно на него самый вероятный рецидив (первая версия
    # `layout.data_folder` была ровно такой). Сдвиг папки клиники разводит
    # все три сразу и стоит одной строки.
    srv = Server()
    root = srv.dir / "clinic-root"
    root.mkdir()
    srv.extra_env["DENTART_DATA_DIR"] = str(root)
    with srv as s:
        c = Client(s.url).login()
        ...
        # ⛔ Самая дорогая строка всего архива: её читают в тот единственный
        # момент, когда данных на машине больше нет и проверить себя нечем.
        # До 21.09 она вела «рядом с DentPilot.exe» — адреса, которого у
        # обычной установки нет с переезда в Program Files. Распакованная
        # туда копия не доезжает до программы: журнал пуст, ошибки ни одной,
        # и выглядит это как пустая копия, а не как промах инструкции.
        # ⭐ Строка сверяется ЦЕЛИКОМ, а не вхождением: путь папки базы
        # содержал бы путь папки клиники как префикс, и подстрочная проверка
        # позеленела бы на соседнем уровне — ровно на той ошибке, ради
        # которой стенд и разведён.
        res.ok("инструкция называет настоящую папку данных",
               f"\r\n    {root}\r\n" in note,
               f"в CITESTE-MA.txt нет строки с путём {root}")
        res.ok("названа папка клиники, а не папка базы",
               str(s.dir) not in note.replace(str(root), ""),
               "в инструкцию уехала папка базы (мимо на уровень)")
        res.ok("инструкция не зовёт восстанавливать в папку программы",
               "folderul programului" not in note,
               "шаг восстановления снова ведёт в папку программы")
        # на НОВОМ компьютере папка может быть другой (портативная раскладка),
        # поэтому путь исходной машины — справка, а авторитет — экран той
        # машины, куда восстанавливают
        res.ok("инструкция отсылает туда, где путь виден",
               "Stare sistem" in note,
               "свериться с настоящей папкой на новом ПК негде")
        # ⚠️ Файл читают Блокнотом на чужой машине: диакритика и одиночные
        # \n ему не годятся. Путь клиники — единственное, что приходит
        # извне, и на ASCII его не проверяем: папка может зваться по-русски.
        body = note.replace(str(root), "")
        res.ok("CITESTE-MA остаётся ASCII без диакритики",
               all(ord(ch) < 127 for ch in body),
               "не-ASCII: " + "".join(sorted({c for c in body if ord(c) > 126})))
        res.ok("CITESTE-MA целиком в CRLF",
               note.count("\n") == note.count("\r\n"),
               "есть голые \\n — Блокнот склеит строки в одну")
