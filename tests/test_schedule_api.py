...
def suite_dash_flag(res: Result) -> None:
    ...
    with s2:
        ...
        res.check("отказ в правах виден СРАЗУ, и его печатает сервер",
                  ...
                  (True, False, True))
        # ⭐ И шапка дня тоже серверная: ссылки «Zi»/«Săptămâna» и выбор даты
        # работают до того, как бандл загрузился — как у девяти соседних
        # экранов.
        res.check("шапка дня на месте и ведёт на неделю",
