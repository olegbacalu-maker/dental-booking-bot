...
def cmd_check(argv: list) -> int:
    ...
    # разом: 08-17 так уехал 1.13.0, указывающий на другой коммит, чем v1.13.0.
    # Отсюда предупреждение здесь, а не только строка в RELEASE.md: правило,
    # которое держится на памяти человека, однажды не сработает.
    # ⚠️ Это [ок] с оговоркой, а не [!!]: выпуску оно не мешает, а красное на
    # том, что чинить не обязательно, перестают читать (грабля жёлтого ok_past).
    stray = [t for t in git("tag", "--list").split()
             if re.fullmatch(r"\d+\.\d+\.\d+", t)]
    if stray:
        say(True, f"локальные теги без «v»: {', '.join(sorted(stray))} — "
                  f"отправлять тег ПОИМЁННО (git push origin vX.Y.Z), "
                  f"«--tags» утащит и их")
