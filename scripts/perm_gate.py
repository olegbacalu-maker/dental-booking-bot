...
⛔ Пара: `routeLoader` без ветки `no_access` оставляет вкладку в разделе с
плашкой отказа — стенд обязан это видеть (красное). Поведение общее для всех
загрузчиков (B3.2): `no_access` у загрузчика значит ровно «маршрут под правом».
"""
